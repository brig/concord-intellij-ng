...
*   **Save console output to log file:** Writes the Run console output to a file for long executions or post-run analysis.
...
For flows with large output, enable **Save console output to log file** in the run configuration. The plugin continues to show output in the **Run** tool window and also writes the same text to a UTF-8 log file.

Use **Log file or directory** to choose where logs are written:

*   Leave it empty to create timestamped logs under `.concord/run-logs` in the working directory.
*   Select a directory to create timestamped logs inside that directory.
*   Select a file to write directly to that file.

When the process starts, the Run console prints the exact log path, so you can open the saved output even if the console buffer later drops older lines.
