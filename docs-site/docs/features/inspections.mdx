...
Detects dependencies that use version strings like `latest` or `PROJECT_VERSION` that can only be resolved by the Concord server, not by the IDE locally. Offers a [quick fix](./quick-fixes#5-add-to-idea-profile) to add a concrete version to the `idea` profile.
