...
Each suggestion shows its **source** on the right side, so you know where the variable comes from:

*   **built-in** — Concord runtime variables like `txId`, `workDir`, `initiator`, `processInfo`, etc. Built-in variables also show a short description.
*   **argument** — Variables declared in `configuration.arguments`.
*   **flow in** — Input parameters defined in the flow's [Documentation Block](./flow-docs).
...
*   **loop** — Loop iteration variables (`item`, `items`, `itemIndex`) available inside <span className="tokenBadge" data-kind="step">loop</span> steps. Loop variables also show a short description.
...
<figure className="docFigure docFigure--md">
    ...
    <figcaption>
        Variable completion inside an EL expression, showing source labels for each suggestion.
    </figcaption>
</figure>
