...
configuration:
  dependencies:
    #  Version 'latest' can only be resolved by the Concord server.
    - "mvn://com.walmartlabs.concord.plugins.basic:smtp-tasks:latest"
```

### The `idea` Profile Approach

The recommended solution is to add a concrete version of the dependency to a special
<span className="tokenBadge" data-kind="section">profiles</span> → `idea` section. The plugin resolves
dependencies from all profiles, including `idea`, so this gives the IDE what it needs without
...
3. Select **Add to idea profile with concrete version**.
4. Enter the desired version in the dialog that appears.

The plugin will create the `profiles.idea.configuration.dependencies` structure automatically
if it doesn't exist, or append to the existing list.

<!-- TODO: screenshot of the inspection warning + quick fix in action -->

:::tip
Once a dependency with the same `groupId:artifactId` exists in the `idea` profile,
