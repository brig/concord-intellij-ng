...
class YAMLFormattingContext {
    ...
    YAMLFormattingContext(@NotNull CodeStyleSettings settings, @NotNull PsiFile file) {
        ...
        CommonCodeStyleSettings common = mySettings.getCommonSettings(ConcordLanguage.INSTANCE);
        mySpaceBuilder = new SpacingBuilder(mySettings, ConcordLanguage.INSTANCE)
    ...
    private Spacing getSpacingAfterSequenceMarker(Block child1, Block child2) {
        ...
        int indentSize = mySettings.getIndentSize(ConcordFileType.INSTANCE);
        if (indentSize < 2) {
            indentSize = 2;
        }
    ...
    private @Nullable Indent computeTemplateIndent(TextRange nodeTextRange) {
        ...
        if (!StringUtilRt.isEmptyOrSpaces(document.getCharsSequence().subSequence(lineStartOffset, nodeTextRange.getStartOffset())))
            return null;
