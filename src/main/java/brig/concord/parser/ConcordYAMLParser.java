...
public class ConcordYAMLParser implements PsiParser, LightPsiParser, YAMLTokenTypes {
    ...
    private void parseNodeProperties() {
        ...
        while (getTokenType() == YAMLTokenTypes.TAG || getTokenType() == YAMLTokenTypes.AMPERSAND) {
            if (getTokenType() == YAMLTokenTypes.AMPERSAND) {
                ...
                if (errorMarker != null) {
                    errorMarker.error(ConcordBundle.message("YAMLParser.multiple.anchors"));
                }
            } else { // tag case
                if (tagWasRead) {
                    PsiBuilder.Marker errorMarker = mark();
                    advanceLexer();
                    errorMarker.error(ConcordBundle.message("YAMLParser.multiple.tags"));
                }
                else {
                    tagWasRead = true;
                    advanceLexer();
    ...
    private @NotNull IElementType parseMultiLineScalar(final IElementType tokenType) {
        ...
        if (getTokenType() == TEXT) {
            ...
            advanceLexer();
            err.error(ConcordBundle.message("YAMLParser.invalid.header.symbols"));
        }
    ...
    private @NotNull IElementType parseArray() {
        ...
        while (!eof()) {
            ...
            if (parsedElement != null) {
                ...
                marker.error(ConcordBundle.message("parsing.error.sequence.item.expected"));
            }
