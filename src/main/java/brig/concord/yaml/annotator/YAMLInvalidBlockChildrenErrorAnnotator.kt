...
private class YAMLInvalidBlockChildrenErrorAnnotator : Annotator, DumbAware {
    override fun annotate(element: PsiElement, holder: AnnotationHolder) {
        ...
        if (element is YAMLBlockMappingImpl) {
            ...
            element.children.firstOrNull { !isValidBlockMapChild(it) }?.let {
                reportSubElementProblem(holder, ConcordBundle.message("inspections.invalid.child.in.block.mapping"), it)
            }

            checkIndent(element.keyValues.toList(), holder, ConcordBundle.message("inspections.invalid.key.indent"))
        }
        if (element is YAMLBlockSequenceImpl) {
            ...
            element.children.firstOrNull { !isValidBlockSequenceChild(it) }?.let {
                reportSubElementProblem(holder, ConcordBundle.message("inspections.invalid.child.in.block.sequence"), it)
            }

            checkIndent(element.items, holder, ConcordBundle.message("inspections.invalid.list.item.indent"))
        }
    }
    ...
    private fun getMessageForParent(element: PsiElement) =
        if (findNeededParent(element) is YAMLKeyValueImpl)
            ConcordBundle.message("inspections.invalid.child.in.block.mapping")
